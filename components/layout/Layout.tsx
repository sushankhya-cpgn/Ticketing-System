    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Page Content */}
          <main className="flex-1 p-6 overflow-auto" style={{ background: "var(--background)" }}>
            {children}
          </main>

          {/* Right Mini Sidebar */}
          <div
            className={`relative flex flex-col transition-width duration-300 ${
              isMiniSidebarOpen ? "w-16" : "w-0"
            }`}
            style={{
              background: "var(--background-minisidebar)",
              borderLeft: "1px solid var(--text-muted)",
          ...
          >
            {isMiniSidebarOpen && (
              <aside className="flex flex-col items-center py-4 space-y-6">
                {/* Toggle Button */}
                <button
                  onClick={() => setIsMiniSidebarOpen(false)}
                  className="p-2 rounded-md hover:bg-[var(--background)] transition"
                  style={{ color: "var(--text-foreground)" }}
                >
                  <svg
                    width="16"
                ...
                <div className="w-8 h-px" style={{ background: "var(--text-muted)", opacity: 0.3 }} />

                {/* Example icons */}
                <button className="p-2 rounded-md hover:bg-[var(--background)] transition">📊</button>
                <button className="p-2 rounded-md hover:bg-[var(--background)] transition">⚙️</button>
                <button className="p-2 rounded-md hover:bg-[var(--background)] transition">❓</button>
              </aside>
            )}
